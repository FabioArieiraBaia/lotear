...
function handleRegistrarPagamento() {
    ...
    $extraAnotacoes = [];
    if ($juros > 0) $extraAnotacoes[] = "Juros: " . number_format($juros, 2, ',', '.') . "%";
    if ($multa > 0) $extraAnotacoes[] = "Multa: " . number_format($multa, 2, ',', '.') . "%";
    if ($desconto > 0) $extraAnotacoes[] = "Desconto: " . number_format($desconto, 2, ',', '.') . "%";
...
function handleGetResumoFinanceiro() {
    ...
    $stmtUltimos = $db->query('
        ...
        FROM pagamentos
        LEFT JOIN lotes ON pagamentos.loteId = lotes.id
        LEFT JOIN corretores ON pagamentos.corretorId = corretores.id OR lotes.corretorId = corretores.id
